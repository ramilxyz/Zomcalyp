...
class Car(private val mRegularMaxSpeed: Float, private val mDrift: Float, private val mAcceleration: Float, mapLoader: MapLoader, wheelDrive: Int, world: World, private val control: ArrayList<Actor>, zgame: ZGame, mstage: Stage) : Actor() {
    ...
    private var sprite = Sprite(zgame.assetManager.get("cars/y0.png", Texture::class.java))
    val body: Body?
    private val mId: Int
    val mstage: Stage
    val zgame: ZGame
    ...
    private fun processInput() {
        ...
        if (mDriveDirection == DRIVE_DIRECTION_FORWARD) {
            baseVector[0f] = mAcceleration * FORWARD_POWER
        } else
            if (mDriveDirection == DRIVE_DIRECTION_BACKWARD) {
                if (direction() == DIRECTION_BACKWARD) {
                    baseVector[0f] = -mAcceleration * REVERSE_POWER
                } else if (direction() == DIRECTION_FORWARD) {
                    baseVector[0f] = -mAcceleration * BREAK_POWER
                } else {
                    baseVector[0f] = -mAcceleration
                }
            }
